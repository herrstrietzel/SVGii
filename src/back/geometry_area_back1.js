...
import { pointAtT, svgArcToCenterParam, getAngle } from "./geometry";
        ...
        pathData.forEach(function (com, i) {
            ...
            if (values.length) {
                ...
                // A commands
                    ...
                    let areafullCircle = Math.PI * rx * ry
                    let arcarea = areafullCircle * rat

                    arcarea -= getPolygonArea([p0, { x: cx, y: cy }, p]);
...
export function getBezierAreaAccuracy(cpts=[], areaPath = 0, areaPoly = 0, tolerance = 0.75) {
    ...
    let comArea = getPathArea(com);
    let areaDiff = getRelativeAreaDiff(areaPath, comArea);
    let accurate = false;

    // sign changes to prevent flipped control points
    if (areaDiff < tolerance) {
        let cpArea = getPolygonArea([p0, cp1, p]);
        let signChange = (cpArea < 0 && areaPoly > 0) || (cpArea > 0 && areaPoly < 0);
        if (signChange) {
    ...
    accurate = areaDiff < tolerance * 0.3;

    return { accurate, areaDiff, comArea }
}
 ...
 * get bezier area
 */
export function getBezierArea(pts) {
