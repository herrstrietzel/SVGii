...
export function simplifyBezierSequence(chunk, thresh, tolerance = 7.5, keepDetails = true) {
    ...
    let vertices = getPathDataVertices(pathDataChunk);
    let polyArea0 = getPolygonArea(vertices);
    ...
    if (Clen > 2) {
        ...
        if (cp1_Int) {
            ...
            let comAccuracy = getBezierAreaAccuracy(p0, cp1_cubic, cp2_cubic, p_2, area0, tolerance );
            //console.log('comAccuracy', comAccuracy);

            accurate = comAccuracy.accurate;
            areaDiff = comAccuracy.areaDiff
    ...
    // combine 2 cubic segments
        ...
        */
        ...
        //comAccuracy1 = getBezierAreaAccuracy(p0, cp1_cubic, cp2_cubic, p_1, area0, tolerance );
        comAccuracy1 = getBezierAreaAccuracy(p0, cp1_cubic, cp2_cubic, p_2, area0, tolerance );


        console.log('!!!simpl', comAccuracy1);


        accurate = comAccuracy1.accurate;
        areaDiff = comAccuracy1.areaDiff
        accuracy1 = areaDiff;
        cp1_cubic_1 = cp1_cubic
        cp2_cubic_1 = cp2_cubic
        ...
        if (areaDiff < tolerance) {
            console.log('try1: ', t, cp1_cubic, cp2_cubic, areaDiff, 'areas', area0, areaSimple, comAccuracy1);

            // check negative angles
            let ang1 = getAngle(p0, cp1_cubic );
            let ang2 = getAngle(p0, cp2_cubic );

            if(ang1<0 && ang2>0 || ang1>0 && ang2<0 ){
                renderPoint(svg1, cp1_cubic, 'cyan' )
                //renderPoint(svg1, cp1, 'blue' )
        ...
        if (!accurate) {
            ...
            //comAccuracy2 = getBezierAreaAccuracy(p0, cp1_cubic, cp2_cubic, p_1, area0, tolerance );
            comAccuracy2 = getBezierAreaAccuracy(p0, cp1_cubic, cp2_cubic, p_2, area0, tolerance );

            accurate = comAccuracy2.accurate;
            areaDiff = comAccuracy2.areaDiff
            accuracy2 = areaDiff;
            cp1_cubic_2 = cp1_cubic
            cp2_cubic_2 = cp2_cubic

            //console.log('accuracy av:', accuracy1, accuracy2, 'areaDiff2', comAccuracy1, comAccuracy2, area0);
    ...
    let polyAreaF = getPolygonArea([p0, cp1_cubic, cp2_cubic, p_2]);
    ...
    if (areaDiff < tolerance) {
        ...
        /*
        //&& areaSimple===242.58583214160336
        if(areaSimple>area0 ){
            renderPoint(svg1, p0, 'green')
            renderPoint(svg1, cp1_cubic, 'orange')
            renderPoint(svg1, cp2_cubic, 'magenta')
            renderPoint(svg1, p_2, 'purple')


        }
            */

        console.log('!!! IS simplified!!!', area0, areaSimple, 'polyAreaF', polyAreaF, polyArea0);
