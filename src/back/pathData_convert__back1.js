...
export function pathDataCubicToArc(pathData) {
    ...
    for (let i = 1; i < pathData.length; i++) {
        ...
        if (type === 'C') {
            ...
            let rx = getDistance(p0, pI);
            let ry = getDistance(p, pI);
            let aspect = rx / ry;
            ...
            arcSegs.push({ index: i,  pt: { x: p.x, y: p.y }, rx, ry, aspect, deltaAngle, sweep });
            ...
            /*
            let len1 = getDistance(p0, cp1);
            let len2 = getDistance(cp2, p);
            //let lenDiffRel = Math.abs(len1 - len2) / ((len1 + len2) / 2);

            if (lenDiffRel <= thresh) {
                let pI = checkLineIntersection(p0, cp1, p, cp2, false);

                if (pI) {
                    let lenCp1 = getDistance(p0, pI);
                    let lenCp2 = getDistance(p, pI);

                    let rx = lenCp1;
                    let ry = lenCp2;
                    let aspect = rx/ry

                    console.log('radii', ry, ry, aspect);

                    let rat1 = len1 / lenCp1;
                    let rat2 = len2 / lenCp2;

                    if (Math.abs(k - rat1) <= thresh && Math.abs(k - rat2) <= thresh) {
                        let r = (lenCp1 + lenCp2) / 2;
                        let ang1 = getAngle(p0, cp1);
                        let ang2 = getAngle(cp2, p);
                        if (ang1 < 0 || ang1 > ang2) ang1 += Math.PI * 2;
                        let delta = ang2 - ang1;
                        let sweep = getPolygonArea([p0, cp1, cp2, p]) > 0 ? 1 : 0;

                        arcSegIndices.push(i);
                        arcSegs.push({ index: i, r, pt: { x: p.x, y: p.y }, rx, ry, aspect, delta, sweep });
                    }
                }
            }
            */
    ...
    for (let i = 1; i < pathData.length; i++) {
        ...
        if (type === 'C' && arcSegIndices.includes(i)) {
            ...
            if (count === 4) {
                ...
                let arcCom1 = { type: 'A', values: [avgRadius, avgRadius, 0, 1, arcGroup[0].sweep, arcGroup[midIdx - 1].pt.x, arcGroup[midIdx - 1].pt.y], decimals };
                let arcCom2 = { type: 'A', values: [avgRadius, avgRadius, 0, 1, arcGroup[midIdx].sweep, arcGroup[arcGroup.length - 1].pt.x, arcGroup[arcGroup.length - 1].pt.y], decimals };
                ...
                let arcCom = { type: 'A', values: [avgRadius, avgRadius, 0, 1, lastArc.sweep, lastArc.pt.x, lastArc.pt.y], decimals };
                ...
                for (let arcSeg of arcGroup) {
                    let arcCom = { type: 'A', values: [arcSeg.r, arcSeg.r, 0, 0, arcSeg.sweep, arcSeg.pt.x, arcSeg.pt.y], decimals };
                    pathDataArcs.push(arcCom);
                }
            }
            i--;
