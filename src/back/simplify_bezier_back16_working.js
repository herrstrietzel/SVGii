...
export function simplifyBezierSequence(chunk, thresh, tolerance = 7.5, keepDetails = true) {
    ...
    if (Clen > 2) {
        ...
        if (cp1_Int) {
            ...
            // test accuracy
            comAreaDiff1 = getBezierAreaAccuracy(p0, cp1_cubic_1, cp2_cubic_1, p_2, area0, tolerance);
        ...
        if (!accurate) {
            ...
            let cp2_mid = type === 'C' ? ptMid.cpts[3] :  ptMid.cpts[1];
            ...
            /*
            renderPoint(svg1, ptMid, 'orange')
            renderPoint(svg1, cp1, 'cyan')
            renderPoint(svg1, cp2_mid, 'blue')
            renderPoint(svg1, cp1_mid, 'magenta')
            renderPoint(svg1, cp1_mid, 'blue')
            renderPoint(svg1, cp2_mid, 'purple')
            ...
            */
            ...
            // test accuracy
            comAreaDiff2 = getBezierAreaAccuracy(p0, cp1_cubic_2, cp2_cubic_2, p_2, area0, tolerance);
    ...
    console.log('diffs', areaDiff, areaDiff1, areaDiff2);
