...
export function simplifyPathData(pathData) {
    ...
    pathDataPlus = optimizePathDataPlusOrder(pathDataPlus);
    //console.log('pathDataPlusRearranged', pathDataPlusRearranged);

    // add chunks to path object
    let pathDataPlusChunks = getPathDataPlusChunks(pathDataPlus);
    ...
    for (let s = 0, l = pathDataPlusChunks.length; l && s < l; s++) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if(type==='L' && chunk.length>1){
                ...
                let simplified = simplifyLinetoSequence(chunk, thresh);
