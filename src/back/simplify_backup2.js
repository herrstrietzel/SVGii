...
export function getPathDataChunks(pathData) {
    ...
    for (let c = 1, len = pathData.length; len && c < len; c++) {
        ...
        if (type === 'Q' || type === 'C') {
            ...
            //showPts = true;
            if(showPts) {
                let d=`M${p0.x} ${p0.y} ${type} ${cp1.x} ${cp1.y} ${p.x}  ${p.y}`;
                //renderPath(svg1, d, 'green', '1%')
            }
            //renderPoint(svg1, cp1, 'orange', '1.5%', '1', lastType + '-' + type)
            ...
            if ((type === 'Q' && typeN === 'Q') || (type === 'C' && typeN === 'C')) {
                ...
                let extCp1 = Math.abs((ang1 % PIquarter)) < tolerance || Math.abs((ang1 % PIquarter) - PIquarter) < tolerance;
                ...
                if (extCp1 || extCp2) {
                    if(showPts){
                        if (extCp1) renderPoint(svg1, cp1, 'orange', '1%')
                    ...
                    if (isCorner) {
                        ...
                        //renderPoint(svg1, p, 'magenta', '2%')

                        com.corner = true
                        com.split = true
                        //com.oi = true
                        console.log('corner', com.values, com);
                    }
